...
public class DoGFilter {
    ...
    private int[] getRasterLineRGB(int[] src, int y, int width, int[] pixel){
    	...
    	for(int i = 0;i < width * 4;i += 4){
    		pixel[i] = ColorUtils.alpha(src[j]);
    		pixel[i + 1] = ColorUtils.red(src[j]);
    		pixel[i + 2] = ColorUtils.green(src[j]);
    		pixel[i + 3] = ColorUtils.blue(src[j]);
    		j++;
    	}
    	return pixel;
    }
    private void setRasterLineRGB(int[] dst, int y, int width, int[] pixel){
    	int j = width * y;
    	for(int i = 0;i < width * 4;i += 4){
    		dst[j] = ColorUtils.argb(pixel[i], pixel[i + 1], pixel[i + 2], pixel[i + 3]);
    		j++;
    	}
    }
