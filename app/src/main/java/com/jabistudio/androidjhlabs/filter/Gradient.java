...
import com.jabistudio.androidjhlabs.filter.util.ColorUtils;
...
public class Gradient extends ArrayColormap implements Cloneable {
	...
	public Object clone() {
		...
		return g;*/
                ...
                throw new UnsupportedOperationException("mnr 2016-06-23: Doesn't build in HTML :(");
	}
	...
	private void rebuildGradient() {
		...
		for (int i = 1; i < numKnots-1; i++) {
			...
			for (int j = xKnots[i]; j < end; j++) {
				...
				ColorUtils.RGBToHSV((rgb1 >> 16) & 0xff, (rgb1 >> 8) & 0xff, rgb1 & 0xff, hsb1);
				float hsb2[] = new float[3];
				ColorUtils.RGBToHSV((rgb2 >> 16) & 0xff, (rgb2 >> 8) & 0xff, rgb2 & 0xff, hsb2);
...
//					if (blend != SPLINE) {
						...
						case HUE_CCW:
							...
							*/
							...
							map[j] = 0xff000000 | ColorUtils.HSVToColor(hsv);//FIXME-alpha
							break;
						}
//					}
				}
